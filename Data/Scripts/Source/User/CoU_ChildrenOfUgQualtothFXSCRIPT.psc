Scriptname CoU_ChildrenOfUgQualtothFXSCRIPT extends ActiveMagicEffect
...
Form Property ElderOfUgQualtoth Auto
...
EffectShader Property CoU_DeathFXS Auto
...
Event OnEffectStart(Actor akTarget, Actor akCaster)
	...
	if  (akTarget as objectReference).WaitFor3DLoad()
		...
		sitState = akTarget.GetSitState()
		if (sleepState == 3)
			;Low glow in ambushes...set to 1.0 if you decide you want no glow
			akTarget.SetSubGraphFloatVariable("fToggleBlend", 1.0)
		elseif (sitState == 3)
			;no glow
			akTarget.SetSubGraphFloatVariable("fToggleBlend", 1.0)
		else
		...
		BaseValueAnim = akTarget.GetBaseValue(AnimationMult)
		BaseValueSpeed = akTarget.GetBaseValue(SpeedMult)
		StartTimer(1.0, 2)
	else
		;nothing
	endif
...
Event OnDeath(Actor akKiller)
	...
	If (Marker.isDestroyed() == false && deathCount < 3)
		...
		If (Marker.isDestroyed() == false)
			...
			ElseIf (aeCombatState == 1)
				...
				While (i < Game.GetPlayerFollowers().length)
					...
					i += 1
				EndWhile
				selfRef.SetValue(SpeedMult, 400.0)
